...
AudioReceiver::AudioReceiver(ServerUser &receiver, Nox::Protocol::audio_context_t context,
...
Nox::Protocol::audio_context_t AudioReceiver::getContext() const {
	return m_context;
}

void AudioReceiver::setContext(Nox::Protocol::audio_context_t context) {
	m_context = context;
}
...
void AudioReceiverBuffer::addReceiver(const ServerUser &sender, ServerUser &receiver,
									  Nox::Protocol::audio_context_t context, bool positionalDataAvailable,
									  const VolumeAdjustment &volumeAdjustment) {
...
void AudioReceiverBuffer::forceAddReceiver(ServerUser &receiver, Nox::Protocol::audio_context_t context,
...
void AudioReceiverBuffer::preprocessBuffer(std::vector< AudioReceiver > &receiverList) {
	...
	std::sort(receiverList.begin(), receiverList.end(), [](const AudioReceiver &lhs, const AudioReceiver &rhs) {
		...
		if (!Nox::Protocol::protocolVersionsAreCompatible(lhs.getReceiver().m_version,
