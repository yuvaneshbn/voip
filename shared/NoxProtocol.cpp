...
#undef PROCESS_NOX_UDP_MESSAGE
	...
	template< Role role > void UDPAudioEncoder< role >::prepareAudioPacket_legacy(const AudioData &data) {
		...
		if (!stream.isValid()) {
			qWarning("NoxProtocol: Encoding legacy packet (fixed part) overflowed buffer size");
			m_staticPartSize = 0;
		}
	...
	template< Role role > void UDPAudioEncoder< role >::addPositionalData_legacy(const AudioData &data) {
		if (data.containsPositionalData) {
			...
			if (!stream.isValid()) {
				   qWarning("NoxProtocol: Adding positional data to legacy packet overflowed buffer size");
				m_positionalAudioSize = m_staticPartSize;
			}
		}
	}
	...
	std::size_t writeSnippet(std::span< const byte > source, std::vector< byte > &destination, std::size_t offset,
		...
		if (maxPacketSize < offset + source.size()) {
			qWarning("NoxProtocol: Buffer overflow while writing snippet. Max buffer size is %zu and required size "
					 "is %zu",
	...
	std::span< const byte > UDPAudioEncoder< role >::updateAudioPacket_protobuf(const AudioData &data) {
		...
		if (offset == 0) {
			qWarning("NoxProtocol: Can't update a packet that hasn't been prepared yet");
			return {};
		}
		...
		qWarning("NoxProtocol: Reached theoretically unreachable code");
		return {};
	}
