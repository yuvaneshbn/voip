...
ServerHandler::ServerHandler() : database(new Database(QLatin1String("ServerHandler"))) {
	...
	//
	// Now, OpenSSL is initialized in main() via NoxSSL::initialize(),
...
void ServerHandler::udpReady() {
	while (qusUdp->hasPendingDatagrams()) {
		char encrypted[Nox::Protocol::MAX_UDP_PACKET_SIZE];
		unsigned int buflen = static_cast< unsigned int >(qusUdp->pendingDatagramSize());

		if (buflen > Nox::Protocol::MAX_UDP_PACKET_SIZE) {
		...
		std::span< Nox::Protocol::byte > buffer = m_udpDecoder.getBuffer();
		...
		if (m_udpDecoder.decode(buffer.subspan(0, buflen - 4))) {
			switch (m_udpDecoder.getMessageType()) {
				...
				case Nox::Protocol::UDPMessageType::Audio: {
					const Nox::Protocol::AudioData audioData = m_udpDecoder.getAudioData();
...
void ServerHandler::handleVoicePacket(const Nox::Protocol::AudioData &audioData) {
	if (audioData.usedCodec != Nox::Protocol::AudioCodec::Opus) {
	...
	if (ao && sender
		&& !((audioData.targetOrContext == Nox::Protocol::AudioContext::WHISPER) && Global::get().s.bWhisperFriends
...
void ServerHandler::sendMessage(const unsigned char *data, int len, bool force) {
	...
	if (!force && (NetworkConfig::TcpModeEnabled() || !bUdp)) {
		...
		*reinterpret_cast< quint16 * >(&uc[0]) =
			qToBigEndian(static_cast< quint16 >(Nox::Protocol::TCPMessageType::UDPTunnel));
		*reinterpret_cast< quint32 * >(&uc[2]) = qToBigEndian(static_cast< quint32 >(len));
		memcpy(uc + 6, data, static_cast< std::size_t >(len));

		QApplication::postEvent(this,
								new ServerHandlerMessageEvent(qba, Nox::Protocol::TCPMessageType::UDPTunnel, true));
	} else {
...
void ServerHandler::sendProtoMessage(const ::google::protobuf::Message &msg, Nox::Protocol::TCPMessageType type) {
	QByteArray qba;
...
void ServerHandler::sendPingInternal() {
	...
	if (qusUdp) {
		Nox::Protocol::PingData pingData;
		pingData.timestamp                    = t;
		pingData.requestAdditionalInformation = false;

		m_udpPingEncoder.setProtocolVersion(m_version);
		std::span< const Nox::Protocol::byte > encodedPacket = m_udpPingEncoder.encodePingPacket(pingData);
...
void ServerHandler::message(Nox::Protocol::TCPMessageType type, const QByteArray &qbaMsg) {
	const char *ptr = qbaMsg.constData();
	if (type == Nox::Protocol::TCPMessageType::UDPTunnel) {
		...
		if (m_tcpTunnelDecoder.decode(
				{ reinterpret_cast< const Nox::Protocol::byte * >(ptr), static_cast< std::size_t >(qbaMsg.size()) })
			&& m_tcpTunnelDecoder.getMessageType() == Nox::Protocol::UDPMessageType::Audio) {
			handleVoicePacket(m_tcpTunnelDecoder.getAudioData());
		}
	} else if (type == Nox::Protocol::TCPMessageType::Ping) {
		NoxProto::Ping msg;
...
void ServerHandler::requestBanList() {
	NoxProto::BanList mpbl;
	mpbl.set_query(true);
	sendMessage(mpbl);
}

void ServerHandler::requestUserList() {
	NoxProto::UserList mpul;
	sendMessage(mpul);
}
...
QUrl ServerHandler::getServerURL(bool withPassword) const {
	...
	url.setScheme(QLatin1String("nox"));
	url.setHost(qsHostName);
	if (usPort != DEFAULT_NOX_PORT) {
		url.setPort(usPort);
	}
